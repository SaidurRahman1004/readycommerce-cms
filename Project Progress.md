...
- **Phase 3 Catalog (In Progress)**: Added public catalog read APIs for products, categories, variants and inventory with category/price filtering, sorting and pagination; added an idempotent premium catalog seeder and connected the Shop/PDP to the backend with loading skeletons, API error states and responsive layouts. Seeding and live API verification remain pending because local MongoDB is currently unreachable.
