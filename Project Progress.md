...
* **[2026-09-05]**: **[Phase 12J Admin Access Fix]**: Audited the dashboard lockout and confirmed the backend authorization response is caused by the account role not being `admin`, not by the HttpOnly cookie transport. Added `packages/backend/make-admin.js` and `npm run make-admin -- user@email.com` to promote an existing account safely. Branch merge status is documented after verification.
