...
- [x] **Manuals & Customer Guides (2026-09-08)**: Added the backend `Manual` model with staff SOP/customer guide types, active/draft status, product relationships, protected admin CRUD, and entitlement-aware customer guide retrieval based on completed/non-cancelled order items. Added the responsive Dashboard Manuals CMS with Markdown-friendly editor, preview, product targeting, loading/empty/error states, and added the Storefront account `/account/guides` reader with mobile accordion layout and server-filtered guides.
...
* **[2026-09-09]**: **[Manuals & SOPs API Resolution & UI/UX Re-Architecture]**: Resolved the "Manuals could not be loaded" API error by fixing the query string serialization in `manualService.list` (eliminating trailing question mark `/admin/manuals?`), expanding route permissions in `adminManualRoutes.js` to all staff roles (`STAFF_ROLES`), and ensuring controller returns `{ success: true, data: [] }` with full population. Re-architected `/manuals` to separate list and form views: `/manuals` now exclusively displays the Data Table with metric cards (Total, Staff SOPs, Customer Guides, Live & Active), search and multi-tag filtering, quick 1-click status toggles, inline quick-preview modal, and delete actions. Routed "+ New manual" to dedicated `/manuals/new` and `/manuals/[id]/edit` pages. Built a premium Markdown Content Editor with formatting toolbar (bold, italic, strikethrough, headings, lists, blockquotes, code, links, tables, rules), live Write/Preview/Split view modes with word and reading-time counters, exact slug helper tooltip (`"Tip: For dashboard contextual help, use slugs like 'page-orders' or 'page-products'."`) with preset pills, and a modern related products multi-select picker with search, thumbnails, price chips, and removal controls. All apps compiled with 0 errors and pushed to `origin/dashboard-all`.
* **[2026-09-09]**: **[Contextual In-App Help Drawer System]**: Implemented an in-app contextual assistance system across the entire Admin Dashboard. Added a Help icon (`HelpCircle`) in the header next to notifications with hover micro-animations and active indicator pulse. Built `ContextualHelpDrawer` utilizing a right slide-out Sheet with backdrop blur, keyboard shortcuts (`Esc` to close, `?` to toggle), and dynamic route detection via `usePathname()`. Maps paths to canonical slugs (`page-[current-path]` like `page-orders`, `page-products`, `page-overview`). If a guide exists, dynamically renders the rich Markdown content, SOP metadata, and direct edit link; if not found (404), displays a sleek Empty State with copyable target slug and a "+ Create Guide for this page" CTA that navigates to `/manuals/new` with pre-filled slug and title. Both apps passed production build checks with 0 errors. Pushed to `origin/dashboard-all`.
* **[2026-09-09]**: **[Contextual Help Drawer & Manuals UI/UX Deep Polish & Bug Fixes]**:
  - Re-architected drawer slide-out mechanics with custom `@keyframes sheetSlideIn` and `.animate-sheet-in` in `globals.css` ensuring fluid animations on both mobile and desktop.
  - Implemented full `.prose` typography stylesheet with modern system font stack, callouts (note, tip, warning), task lists (`- [ ]`, `- [x]`), inline code styling, and responsive code/table wrapping.
  - Redesigned the Help Drawer Empty State with a double-ring compass illustration, clear status badges, 1-click mapped slug copy box, and prominent `+ Create [Page] Guide` CTA button routing to `/manuals/new` with pre-filled slug and title.
