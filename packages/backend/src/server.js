...
app.get('/api/cms/homepage', async (req, res, next) => {
  try {
    const HomepageConfig = require('./models/HomepageConfig');
    const data = await HomepageConfig.findOne({ key: 'homepage' }).populate('featuredCategories', 'name slug image').lean();
    return res.json({ success: true, data: data || { slides: [], featuredCategories: [], promoEnabled: false } });
  } catch (e) {
    return next(e);
  }
});
...
app.get('/api/settings/shipping', async (req, res, next) => {
  try {
    const settings = await StoreSettings.findOne({ key: 'store' }).select('insideDhakaRate outsideDhakaRate currency').lean();
    const city = typeof req.query.city === 'string' ? req.query.city : '';
    ...
    res.json({
      ...
      data: {
        city,
        insideDhakaRate: settings?.insideDhakaRate ?? 60,
        outsideDhakaRate: settings?.outsideDhakaRate ?? 120,
        cost: dhaka ? (settings?.insideDhakaRate ?? 60) : (settings?.outsideDhakaRate ?? 120),
