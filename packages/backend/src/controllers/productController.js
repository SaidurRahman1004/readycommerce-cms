...
const listProducts = async (req, res, next) => {
  try {
    ...
    const filter = { status: 'active' }; const { category, minPrice, maxPrice, search, isSpecialOffer, sort = 'featured', scent, color } = req.query;
    
    const cacheKey = `catalog:products:page=${page}&limit=${limit}&category=${category||''}&minPrice=${minPrice||''}&maxPrice=${maxPrice||''}&search=${search||''}&isSpecialOffer=${isSpecialOffer||''}&sort=${sort}&scent=${scent||''}&color=${color||''}`;
    ...
    const variantFilter = {}; if (scent) variantFilter.scent = String(scent); if (color) variantFilter.color = String(color); if (Object.keys(variantFilter).length) { const matchingVariants = await ProductVariant.find(variantFilter).distinct('product'); filter._id = { $in: matchingVariants }; }
