...
const guestCookie = 'rc_cart_session';
const getCartKey = (req, res) => {
  if (req.user) return { user: req.user._id };
  const sessionId = req.cookies?.[guestCookie] || crypto.randomBytes(24).toString('hex');
  if (!req.cookies?.[guestCookie]) res.cookie(guestCookie, sessionId, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', maxAge: 30 * 86400000, path: '/api/cart' });
  return { sessionId };
};
const serialize = async (cart) => { const items = await CartItem.find({ cart: cart._id }).populate('product', 'name slug images').populate('variant', 'name sku size color price').lean(); return { id: cart._id, items: items.map((item) => ({ id: item._id, productId: item.product._id, variantId: item.variant?._id, quantity: item.quantity, price: item.unitPrice, name: item.productName, image: item.imageSnapshot || item.product.images?.[0], sku: item.sku })), subtotal: cart.subtotal, discount: cart.discount, shipping: cart.shipping, tax: cart.tax, total: cart.total, currency: cart.currency }; };
const findOrCreate = async (req, res) => { const key = getCartKey(req, res); return Cart.findOneAndUpdate({ ...key, status: 'active' }, { $setOnInsert: { ...key, status: 'active', currency: 'BDT' } }, { upsert: true, returnDocument: 'after', setDefaultsOnInsert: true }); };
const recalculate = async (cart) => { const items = await CartItem.find({ cart: cart._id }).lean(); cart.subtotal = items.reduce((sum, item) => sum + item.unitPrice * item.quantity, 0); cart.total = cart.subtotal - cart.discount + cart.shipping + cart.tax; await cart.save(); return cart; };

const getCart = async (req, res, next) => { try { const cart = await findOrCreate(req, res); return res.json({ success: true, data: await serialize(cart) }); } catch (error) { return next(error); } };
