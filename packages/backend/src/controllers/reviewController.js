...
const createReview = async (req, res, next) => {
  try {
    const { productId, rating, body, title } = req.body;
    ...
    if (await Review.exists({ product: productId, user: req.user._id })) return next(new AppError('You have already reviewed this product.', 409, 'REVIEW_EXISTS'));
    ...
    const stats = await Review.aggregate([{ $match: { product: new mongoose.Types.ObjectId(productId), status: 'approved' } }, { $group: { _id: '$product', average: { $avg: '$rating' }, count: { $sum: 1 } } }]);
