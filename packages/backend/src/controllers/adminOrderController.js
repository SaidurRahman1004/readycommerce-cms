...
const bulkUpdateStatus = async (req, res, next) => {
  try {
    ...
    for (const order of orders) {
      if (order.status !== status && (transitions[order.status]?.includes(status) || req.user.role === 'super-admin')) {
        order.status = status;
    ...
    return res.json({ success: true, message: `Successfully updated ${updated} orders to ${status}.` });
  } catch (error) { return next(error); }
};

const exportOrders = async (req, res, next) => { try {
  const { Parser } = require('json2csv');
  const filter = {}; if (statuses.includes(req.query.status)) filter.status = req.query.status; if (paymentStatuses.includes(req.query.paymentStatus)) filter.paymentStatus = req.query.paymentStatus;
  if (typeof req.query.search === 'string' && req.query.search.trim()) { const search = req.query.search.trim(); filter.$or = [{ orderNumber: { $regex: search, $options: 'i' } }, { email: { $regex: search, $options: 'i' } }]; }
  const orders = await Order.find(filter).populate('user', 'firstName lastName email phone').sort({ createdAt: -1 }).lean();
  ...
  const parser = new Parser({ fields: ['OrderID', 'CustomerName', 'CustomerEmail', 'CustomerPhone', 'TotalAmount', 'Currency', 'OrderStatus', 'PaymentStatus', 'Date'] });
  const csv = parser.parse(data);
  res.header('Content-Type', 'text/csv');
  res.attachment(`orders-export-${new Date().toISOString().slice(0,10)}.csv`);
  return res.send(csv);
} catch (error) { return next(error); } };

module.exports = { listOrders, getOrder, updateStatus, updatePayment, exportOrders, bulkUpdateStatus };
