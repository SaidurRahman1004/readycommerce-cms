...
export default function SuccessClient() {
  const t = useTranslations('Success'); const [orderId, setOrderId] = useState('');
  useEffect(() => {const timer = window.setTimeout(() => setOrderId(window.localStorage.getItem('readycommerce_last_order_id') ?? ''), 0); return () => window.clearTimeout(timer);}, []);
