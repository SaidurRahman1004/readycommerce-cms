...
export default function CheckoutClient() {
  ...
  async function submit(event: FormEvent) {
    ...
    try { const created = await orderService.create({ addressId: selectedAddress, paymentMethod: values.payment, txid: values.txid, idempotencyKey: idempotencyRef.current }); window.localStorage.setItem('readycommerce_last_order_id', created.data.orderId); clearCart(); toast.success(t('success')); router.push(`/success?orderId=${encodeURIComponent(created.data.orderId)}`); } catch { toast.error(authT('errors.generic')); } finally { setOrderLoading(false); }
  }
