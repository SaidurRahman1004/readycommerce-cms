...
type Values = { name: string; email: string; phone: string; address: string; city: string; postal: string; payment: 'bkash' | 'nagad' | 'stripe'; txid: string };
const initialValues: Values = { name: '', email: '', phone: '', address: '', city: '', postal: '', payment: 'stripe', txid: '' };
...
export default function CheckoutClient() {
  ...
  async function submit(event: FormEvent) {
    ...
    try { const created = await orderService.create({ addressId: selectedAddress, paymentMethod: values.payment, txid: values.txid, idempotencyKey: idempotencyRef.current }); window.localStorage.setItem('readycommerce_last_order_id', created.data.orderId); clearCart(); if (created.data.paymentUrl) { window.location.href = created.data.paymentUrl; } else { toast.success(t('success')); router.push(`/success?orderId=${encodeURIComponent(created.data.orderId)}`); } } catch { toast.error(authT('errors.generic')); } finally { setOrderLoading(false); }
  }
      ...
      <div className="mx-auto max-w-6xl px-5 py-12 sm:px-8 lg:py-20"><div className="mb-12"><p className="mb-4 text-[13px] font-bold uppercase tracking-[0.25em] text-primary">{t('eyebrow')}</p><h1 className="text-4xl font-bold tracking-tight sm:text-5xl">{t('title')}</h1></div>
        <form onSubmit={submit} noValidate className="grid gap-12 lg:grid-cols-[1fr_420px] lg:gap-16">
          <div className="space-y-10">
            ...
            <section className="rounded-3xl border border-border bg-surface p-6 sm:p-8"><h2 className="text-xl font-bold">{t('payment.title')}</h2><p className="mt-2 text-sm text-muted-foreground">{t('payment.description')}</p><div className="mt-6 grid gap-4 sm:grid-cols-2">{(['stripe', 'bkash', 'nagad'] as const).map((method) => <label key={method} className="flex min-h-14 items-center gap-3 rounded-xl border border-border p-4 font-bold cursor-pointer hover:border-primary/50 transition"><input type="radio" name="payment" value={method} checked={values.payment === method} onChange={() => update('payment', method)} />{method === 'stripe' ? 'Credit Card (Stripe)' : t(`payment.${method}`)}</label>)}</div>{['bkash', 'nagad'].includes(values.payment) && <label className="mt-6 block text-sm font-bold animate-in fade-in slide-in-from-top-2">{t('fields.txid')}<input className={input('txid')} value={values.txid} onChange={(event) => update('txid', event.target.value)} />{errors.txid && <span className="mt-2 block text-xs text-rose-600">{errors.txid}</span>}</label>}</section>
          </div>
