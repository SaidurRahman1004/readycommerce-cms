...
export default function ProductGrid() {
  ...
  return <div className="grid grid-cols-2 gap-x-4 gap-y-8 sm:gap-x-6 lg:grid-cols-4">{products.map((product) => <article key={product.key} className="group"><div className="relative aspect-[4/5] overflow-hidden rounded-2xl bg-[#f1f2f6]"><Image src={product.image} alt={t(`products.${product.key}.name`)} fill sizes="(max-width: 640px) 45vw, (max-width: 1024px) 30vw, 23vw" className="object-cover transition duration-700 group-hover:scale-105" /><button type="button" onClick={() => addItem(product.key === 'noir' ? 'noir-07' : product.key === 'santal' ? 'santal-33' : product.key === 'serum' ? 'botanical-glow' : 'muse-veil')} className="absolute bottom-3 left-3 right-3 hidden min-h-11 items-center justify-center rounded-xl bg-white/95 text-xs font-semibold text-slate-900 shadow-lg backdrop-blur transition hover:bg-indigo-600 hover:text-white sm:flex sm:translate-y-2 sm:opacity-0 sm:group-hover:translate-y-0 sm:group-hover:opacity-100">{t('products.addToCart')}</button></div><div className="mt-4 flex items-start justify-between gap-2"><div><h3 className="text-sm font-semibold text-slate-900 sm:text-[15px]">{t(`products.${product.key}.name`)}</h3><p className="mt-1 text-xs text-slate-500">{t(`products.${product.key}.category`)}</p></div><p className="text-sm font-semibold text-slate-900">{product.price}</p></div><button type="button" onClick={() => addItem(product.key === 'noir' ? 'noir-07' : product.key === 'santal' ? 'santal-33' : product.key === 'serum' ? 'botanical-glow' : 'muse-veil')} className="mt-3 min-h-10 w-full rounded-xl border border-slate-200 text-xs font-semibold text-slate-700 transition hover:border-indigo-600 hover:bg-indigo-600 hover:text-white sm:hidden">{t('products.addToCart')}</button></article>)}</div>;
}
