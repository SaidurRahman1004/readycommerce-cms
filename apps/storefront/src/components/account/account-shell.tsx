...
export default function AccountShell({children}: {children: React.ReactNode}) {
  ...
  const logout = async () => {setLoggingOut(true); try {await clearSession(); await reloadCart(); toast.success(t('logoutSuccess')); router.push('/');} catch {toast.error(t('errors.generic'));} finally {setLoggingOut(false);}};
  ...
  const links = [{href: '/account/profile', key: 'profile'}, {href: '/account/orders', key: 'orders'}, {href: '/account/addresses', key: 'addresses'}, {href: '/account/guides', key: 'guides'}] as const;
