...
export default function AccountShell({children}: {children: React.ReactNode}) {
  const t = useTranslations('Account'); const pathname = usePathname(); const router = useRouter(); const [loggingOut, setLoggingOut] = useState(false);
  const logout = async () => {setLoggingOut(true); try {await authService.logout(); toast.success(t('logoutSuccess')); router.push('/');} catch {toast.error(t('errors.generic'));} finally {setLoggingOut(false);}};
