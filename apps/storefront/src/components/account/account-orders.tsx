...
function OrderTimeline({ status, t }: { status: string; t: (key: string, values?: Record<string, string | number>) => string }) {
  ...
  return <div className="mt-6 rounded-2xl border border-border/80 bg-surface/70 p-4 sm:p-5">
    {terminal ? <div className="flex items-center gap-3 rounded-xl bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-rose-100">!</span>{t(`orders.status.${status}`, { fallback: status })}</div> : <div className="grid grid-cols-4 gap-2 sm:gap-4">
      {TRACKING_STEPS.map((step, index) => { const complete = currentIndex >= index; return <div key={step} className="relative text-center">
        ...
        <span className={`mt-2 block text-[10px] font-semibold sm:text-xs ${complete ? 'text-foreground' : 'text-muted-foreground'}`}>{t(`orders.status.${step}`, { fallback: step })}</span>
      </div>; })}
    </div>}
  </div>;
}
