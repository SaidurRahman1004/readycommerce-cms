...
export const adminOrderService = { list: (params: Record<string, string | number | undefined> = {}) => { const query = new URLSearchParams(Object.entries(params).filter(([, value]) => value !== undefined && value !== '').map(([key, value]) => [key, String(value)])); return request<{ success: boolean; data: AdminOrder[]; pagination: { page: number; limit: number; total: number; pages: number } }>(`/admin/orders?${query}`); }, detail: (id: string) => request<{ success: boolean; data: AdminOrderDetail }>(`/admin/orders/${encodeURIComponent(id)}`), status: (id: string, status: string) => request<{ success: boolean; data: { orderId: string; status: string } }>(`/admin/orders/${encodeURIComponent(id)}/status`, { method: 'PUT', body: JSON.stringify({ status }) }), payment: (id: string, status: string, failureReason?: string) => request<{ success: boolean; data: { orderId: string; paymentStatus: string } }>(`/admin/orders/${encodeURIComponent(id)}/payment`, { method: 'PUT', body: JSON.stringify({ status, failureReason }) }), refund: (id: string, action: 'request'|'approve'|'refund') => request<{success:boolean;data:{orderId:string;status:string;returnStatus?:string;paymentStatus:string}}>(`/admin/orders/${encodeURIComponent(id)}/refund`, {method:'PUT',body:JSON.stringify({action})}) };
